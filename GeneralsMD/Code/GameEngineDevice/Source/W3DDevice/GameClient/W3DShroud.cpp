...
void W3DShroud::init(WorldHeightMap *pMap, Real worldCellSizeX, Real worldCellSizeY)
{
  ...
  //memory texture to a known value because you can't lock it - only copy into it.
	...
	// shroud sysmem surface through SurfaceClass instead of raw surface pointers.
...
void W3DShroud::render(CameraClass *cam)
{
	...
	// TheSuperHackers @refactor bobtista 10/04/2026 Phase 3C: use the
	// backend device-lost flag to skip rendering this frame if the device
	// is not ready.
