...
// dynamic shadow volume buffers in W3D classes so WW3D::Get_Render_Backend()->
...
void W3DVolumetricShadow::RenderMeshVolume(Int meshIndex, Int lightIndex, const Matrix3D *meshXform)
{
	...
	{	lastActiveVertexBuffer=vbSlot->m_VB->m_DX8VertexBuffer->Get_DX8_Vertex_Buffer();
		WW3D::Get_Render_Backend()->Set_Vertex_Buffer(vbSlot->m_VB->m_DX8VertexBuffer, 0);
	}
	...
	WW3D::Get_Render_Backend()->Set_Index_Buffer(ibSlot->m_IB->m_DX8IndexBuffer, vbSlot->m_start);
...
void W3DVolumetricShadowManager::renderShadows( Bool forceStencilFill )
{
	...
	if (m_shadowList && TheGlobalData->m_useShadowVolumes)
	{
		...
		// calls below are routed through WW3D::Get_Render_Backend(). The raw m_pDev->SetRenderState
...
//		m_pDev->SetRenderState(D3DRS_ZBIAS,1);	///@todo: See if this helps or makes things worse.
		...
		WW3D::Get_Render_Backend()->Set_Stencil_Pass_Op(RB_STENCIL_OP_DECR_SAT);
