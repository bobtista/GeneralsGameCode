...
// TheSuperHackers @bugfix bobtista 31/05/2026 Separate dynamic buffers for the
// projected decal list (spy reveal grid, scorch/status decals). The reveal-grid
// pass and the blob/projection shadow pass previously shared the buffers below;
// in the bgfx deferred-submit model the grid pass re-locked the shared buffer and
// overwrote the still-pending blob shadow submits, drawing the blobs as long
// terrain-spanning dark streaks during a reveal. Drawing the decal-list pass from
// its own buffer removes that cross-pass aliasing.
RenderVertexBufferClass * decalListVertexBuffer = nullptr;
RenderIndexBufferClass  * decalListIndexBuffer  = nullptr;
// TheSuperHackers @bugfix bobtista 01/06/2026 Dedicated dynamic buffers for the
// infantry blob (queueSimpleDecal) batch within the m_shadowList pass. Blob decals
// (ShadowI.tga) and other ground decals (shadow.tga etc.) are interleaved in that
// one loop and share the shadowDecal* buffers above. In the bgfx deferred-submit
// model the second texture-batch re-locks the shared buffer at offset 0 and the
// last update wins per offset at frame end, so the still-pending blob draw reads
// the building decal's vertex/index data instead of the blob quads and almost all
// infantry blob shadows vanish. Routing the blob batch through its own buffers
// removes that intra-pass aliasing.
...
Int W3DProjectedShadowManager::renderShadows(RenderInfoClass & rinfo)
{
	...
	if (TheGlobalData->m_useShadowDecals && !TheGlobalData->m_bgfxShadowMaps)
	{
		...
		// TheSuperHackers @bugfix bobtista 01/06/2026 Route the infantry blob batch
		// (queueSimpleDecal) through its own dedicated buffers so it can never alias
		// the shadowDecal* buffers used by the building/other-decal batch (queueDecal)
		// within this same loop. Blob and non-blob SHADOW_DECALs interleave here and
		// are flushed on texture change; without dedicated buffers the second batch
		// re-locks the shared buffer at offset 0 and the still-pending blob draw reads
		// the wrong data at bgfx frame end, so almost all blob shadows vanish. We save
		// the shared buffer globals, then point the globals at the blob buffers while a
		// blob run is active and at the shared buffers while a non-blob run is active,
		// forcing a fresh DISCARD on each switch. The pending batch is always flushed
		// before a switch (blob and non-blob always differ in texture), so a batch
		// never spans two buffers.
