...
void SDL3GameEngine::update()
{
	...
	// TheSuperHackers @bugfix bobtista 08/06/2026 Apply a settled window resize HERE, after the client
	// and window-manager update have finished for the frame. The full relayout (recreateWindowLayouts/
	// recreateControlBar) destroys and rebuilds live GameWindow trees and crashes when run mid
	// event-poll; this between-frames point is the same safe context the options-menu resolution change
	// runs in, so it can do the complete job instead of the cut-down one.
...
void SDL3GameEngine::pollSDL3Events()
{
	...
	// TheSuperHackers @bugfix bobtista 08/06/2026 Sync the engine resolution to the window once it
	// has SETTLED. The game's Display/2D/mouse layer is set from the requested resolution at startup,
	// but the actual window content can differ - notably macOS fullscreen animates through several
	// intermediate sizes before settling on the usable area below the menu bar/notch (e.g. 982 -> 852
	// -> ... -> 949), and a windowed drag-resize changes it live. Applying a mid-transition size
	// resets the render device on a size that is about to change again and crashed; instead wait
	// until SDL_GetWindowSize has been stable for a few polls, then apply once. bgfx already tracks
	// the real size every frame, so the view keeps rendering correctly during the wait.
