...
public:
	...
	Bool resumePlayback( AsciiString filename, UnsignedInt frame );	///< Re-enter playback of a replay from a checkpoint frame: skip commands at or before it.
	Bool isResumedPlayback() const { return m_resumedPlayback; }		///< True while a playback entered through resumePlayback is running.
...
protected:
	...
	UnsignedInt m_resumeMinCRCFrame;								///< Recorded CRCs describing frames before this are stale after a checkpoint resume.
	Bool m_resumedPlayback;													///< The current playback was entered from a checkpoint.
};
