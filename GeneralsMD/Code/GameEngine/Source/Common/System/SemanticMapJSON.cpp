...
// TheSuperHackers @feature bobtista 29/06/2026 Semantic JSON serialization for map files
...
// Object flag bits (FLAG_*) come from Common/MapObject.h so they cannot drift from the engine.

static const char* s_timeOfDayNames[] = {
	"invalid", "morning", "afternoon", "evening", "night"
};
static_assert(sizeof(s_timeOfDayNames) / sizeof(s_timeOfDayNames[0]) == TIME_OF_DAY_COUNT,
	"s_timeOfDayNames must stay in sync with the TimeOfDay enum");

static const char* s_weatherNames[] = {
	"NORMAL", "SNOWY"
};
static const int s_weatherCount = 2;
static_assert(s_weatherCount == WEATHER_COUNT,
	"s_weatherNames must stay in sync with the Weather enum");
...
void MapData::clear()
{
	...
	// TheSuperHackers @bugfix bobtista 29/06/2026 Free per-object property Dicts owned by raw pointer
	for (size_t i = 0; i < objects.size(); i++)
	{
		if (objects[i].properties)
		{
			delete objects[i].properties;
			objects[i].properties = NULL;
		}
	}
	objects.clear();
	waypointLinks.clear();
...
{
	std::string result;
	if (flags & FLAG_DRAWS_IN_MIRROR)
	{
		result += "drawsInMirror,";
	}
	if (flags & FLAG_ROAD_POINT1)
	{
		result += "roadPoint1,";
	}
	if (flags & FLAG_ROAD_POINT2)
	{
		result += "roadPoint2,";
	}
	if (flags & FLAG_ROAD_CORNER_ANGLED)
	{
		result += "roadCornerAngled,";
	}
	if (flags & FLAG_BRIDGE_POINT1)
	{
		result += "bridgePoint1,";
	}
	if (flags & FLAG_BRIDGE_POINT2)
	{
		result += "bridgePoint2,";
	}
	if (flags & FLAG_ROAD_CORNER_TIGHT)
	{
		result += "roadCornerTight,";
	}
	if (flags & FLAG_ROAD_JOIN)
	{
		result += "roadJoin,";
	}
	if (flags & FLAG_DONT_RENDER)
	{
		result += "dontRender,";
	}
...
{
	int flags = 0;
	if (str.find("drawsInMirror") != std::string::npos)
	{
		flags |= FLAG_DRAWS_IN_MIRROR;
	}
	if (str.find("roadPoint1") != std::string::npos)
	{
		flags |= FLAG_ROAD_POINT1;
	}
	if (str.find("roadPoint2") != std::string::npos)
	{
		flags |= FLAG_ROAD_POINT2;
	}
	if (str.find("roadCornerAngled") != std::string::npos)
	{
		flags |= FLAG_ROAD_CORNER_ANGLED;
	}
	if (str.find("bridgePoint1") != std::string::npos)
	{
		flags |= FLAG_BRIDGE_POINT1;
	}
	if (str.find("bridgePoint2") != std::string::npos)
	{
		flags |= FLAG_BRIDGE_POINT2;
	}
	if (str.find("roadCornerTight") != std::string::npos)
	{
		flags |= FLAG_ROAD_CORNER_TIGHT;
	}
	if (str.find("roadJoin") != std::string::npos)
	{
		flags |= FLAG_ROAD_JOIN;
	}
	if (str.find("dontRender") != std::string::npos)
	{
		flags |= FLAG_DONT_RENDER;
	}
	return flags;
}
