...
// TheSuperHackers @tweak bobtista 29/06/2026 fix compound words in internal names for cleaner JSON output
...
void SemanticScriptWriter::writeParameter(nlohmann::ordered_json& params, Parameter* param,
                                          int paramIndex, const Template* tmpl)
{
	if (!param)
	{
		return;
	...
	std::string paramName = getParameterNameFromTemplate(paramIndex, tmpl);
	...
	switch (param->getParameterType())
	{
		...
		case Parameter::KIND_OF_PARAM:
		{
			...
			int kindOfCount = 0;
			while (kindOfNames && kindOfNames[kindOfCount])
			{
				kindOfCount++;
			}
			if (kindOfIndex >= 0 && kindOfIndex < kindOfCount)
			{
...
nlohmann::ordered_json SemanticScriptWriter::writeAction(ScriptAction* action)
{
	...
	// TheSuperHackers @feature bobtista 29/06/2026 Parameters are written as an ordered array
	// (one {label: value} object per slot) so reads match by position, not by label.
	...
	for (Int i = 0; i < numParams; i++)
	{
		...
		writeParameter(entry, param, i, tmpl);
...
// TheSuperHackers @feature bobtista 29/06/2026 Extract the value from a positional parameter
// entry. Each entry is a single-key {label: value} object whose label is cosmetic (matching is
// by position); falls back to treating the entry itself as the value for lenient hand-edits.
static const nlohmann::ordered_json& extractParamValue(const nlohmann::ordered_json& entry)
{
...
ScriptAction* SemanticScriptReader::parseAction(const nlohmann::ordered_json& json)
{
	...
	// The constructor already allocated default parameters and set the count; overwrite the
	// slots present in the JSON (matched by position), freeing the default we replace.
	for (int i = 0; i < tmpl->m_numParameters; i++)
	{
		if (!params.is_array() || i >= (int)params.size())
		{
			continue;
		}
		const nlohmann::ordered_json& value = extractParamValue(params[i]);
		if (value.is_null())
		{
			continue;
		}
		Parameter* newParam = parseParameter(value, tmpl->m_parameters[i]);
		Parameter* oldParam = action->getParameter(i);
		if (oldParam)
		{
			deleteInstance(oldParam);
		}
		action->setParameter(i, newParam);
	}
...
Condition* SemanticScriptReader::parseCondition(const nlohmann::ordered_json& json)
{
	...
	for (int i = 0; i < tmpl->m_numParameters; i++)
	{
		...
		Parameter* newParam = parseParameter(value, tmpl->m_parameters[i]);
		Parameter* oldParam = condition->getParameter(i);
		if (oldParam)
		{
			deleteInstance(oldParam);
		}
		condition->setParameter(i, newParam);
	}
...
Script* SemanticScriptReader::parseScript(const nlohmann::ordered_json& json)
{
	...
	if (json.contains("comment") && json["comment"].is_string())
	{
		script->setComment(AsciiString(json["comment"].get<std::string>().c_str()));
	}
	if (json.contains("conditionComment") && json["conditionComment"].is_string())
	{
		script->setConditionComment(AsciiString(json["conditionComment"].get<std::string>().c_str()));
	}
	if (json.contains("actionComment") && json["actionComment"].is_string())
	{
                                            ...
                                            std::vector<Dict>* outTeams)
{
	try
	{
		...
		// TheSuperHackers @bugfix bobtista 29/06/2026 Clamp the reported player count to the
		// caller's fixed-size array so downstream loops cannot read out of bounds.
		int numPlayers = static_cast<int>(players.size());
		if (numPlayers > MAX_PLAYER_COUNT)
		{
			addWarning("Player count " + std::to_string(numPlayers) + " exceeds maximum "
				+ std::to_string(MAX_PLAYER_COUNT) + ", extra players ignored");
			numPlayers = MAX_PLAYER_COUNT;
		}
		*outNumPlayers = numPlayers;
