	...
	{
		{
					...
					TheGameLogic->isInGame())
			{
				...
				if ((UnsignedInt)(now - s_recoveryWaitStart) >= staggerMs)
				{
					...
					if (!snapshotArrived)
					{
						...
						// TheSuperHackers @bugfix bobtista 08/09/2026 Ask the donor again only once the transfer
					...
					TheNetwork->getRecoveryReceivedFile().isEmpty())
			{
				...
				if (rejoinPercent <= 0 && (UnsignedInt)(requestNow - s_firstRejoinRequest) >= (UnsignedInt)REJOIN_GIVEUP_MS)
				{
