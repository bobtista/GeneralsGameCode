...
void Drawable::xfer( Xfer *xfer )
{
	...
	const XferVersion currentVersion = (xfer->getXferMode() == XFER_SAVE && xfer->getPurpose() != XFER_PURPOSE_CHECKPOINT) ? 5 : 8;
#elif RETAIL_COMPATIBLE_XFER_SAVE
	const XferVersion currentVersion = (xfer->getXferMode() == XFER_SAVE && xfer->getPurpose() != XFER_PURPOSE_CHECKPOINT) ? 7 : 8;
#else
