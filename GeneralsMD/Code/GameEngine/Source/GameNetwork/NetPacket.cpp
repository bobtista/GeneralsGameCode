...
namespace NetPacketFieldTypes {
	const char COMMAND_TYPE = 'T';		// NetCommandType field
	const char RELAY = 'R';				// Relay field
	const char PLAYER_ID = 'P';			// Player ID field
	const char COMMAND_ID = 'C';		// Command ID field
...
NetCommandRef * NetPacket::ConstructNetCommandMsgFromRawData(UnsignedByte *data, UnsignedShort dataLength) {
	...
	while (offset < (Int)dataLength) {
		...
		case NetPacketFieldTypes::COMMAND_TYPE:
			++offset;
		...
		case NetPacketFieldTypes::PLAYER_ID:
			++offset;
			memcpy(&playerID, data + offset, sizeof(UnsignedByte));
			offset += sizeof(UnsignedByte);
			break;

		case NetPacketFieldTypes::COMMAND_ID:
			++offset;
...
UnsignedInt NetPacket::GetWrapperCommandSize(NetCommandMsg *msg) {
	...
	++msglen; // NetPacketFieldTypes::COMMAND_TYPE
	msglen += sizeof(UnsignedByte); // command type
	msglen += sizeof(UnsignedByte) + sizeof(UnsignedByte); // NetPacketFieldTypes::PLAYER_ID and player ID
	msglen += sizeof(UnsignedByte) + sizeof(UnsignedShort); // NetPacketFieldTypes::COMMAND_ID and command ID
...
UnsignedInt NetPacket::GetFileCommandSize(NetCommandMsg *msg) {
	...
	UnsignedInt msglen = 0;
	msglen += sizeof(UnsignedByte) + sizeof(UnsignedByte); // NetPacketFieldTypes::COMMAND_TYPE and command type
	msglen += sizeof(UnsignedByte) + sizeof(UnsignedByte); // NetPacketFieldTypes::PLAYER_ID and player ID
	msglen += sizeof(UnsignedByte) + sizeof(UnsignedShort); // NetPacketFieldTypes::COMMAND_ID and command ID
	msglen += sizeof(UnsignedByte) + sizeof(UnsignedByte); // NetPacketFieldTypes::RELAY and relay
...
void NetPacket::FillBufferWithGameCommand(UnsignedByte *buffer, NetCommandRef *msg) {
	...
	buffer[offset] = NetPacketFieldTypes::COMMAND_TYPE;
	++offset;
	...
	buffer[offset] = NetPacketFieldTypes::PLAYER_ID;
	++offset;
	buffer[offset] = cmdMsg->getPlayerID();
	offset += sizeof(UnsignedByte);

	// If necessary, specify the command ID of this command.
	buffer[offset] = NetPacketFieldTypes::COMMAND_ID;
	++offset;
...
Bool NetPacket::addFrameResendRequestCommand(NetCommandRef *msg) {
	...
	if (isRoomForFrameResendRequestMessage(msg)) {
		...
		if (m_lastCommandType != cmdMsg->getNetCommandType()) {
			m_packet[m_packetLen] = NetPacketFieldTypes::COMMAND_TYPE;
			++m_packetLen;
		...
		if (m_lastPlayerID != cmdMsg->getPlayerID()) {
			m_packet[m_packetLen] = NetPacketFieldTypes::PLAYER_ID;
			++m_packetLen;
		...
		if (((m_lastCommandID + 1) != (UnsignedShort)(cmdMsg->getID())) || (needNewCommandID == TRUE)) {
			m_packet[m_packetLen] = NetPacketFieldTypes::COMMAND_ID;
			++m_packetLen;
			UnsignedShort newID = cmdMsg->getID();
