...
//-------------------------------------------------------------------------------------------------
void OpenALAudioManager::playAudioEvent(const RefCountPtr<DynamicAudioEventRTS>& eventPtr, AudioRequest* req)
{
	AudioEventRTS* event = eventPtr.Peek();
	...
	case AT_Streaming:
	{
		...
		audio->m_audioEventRTS = eventPtr;
	...
	if (lowestPriorityEvent)
	{
		...
		if (event->isPositionalAudio())
		{
			for (it = m_playing3DSounds.begin(); it != m_playing3DSounds.end(); ++it)
			{
				...
				if (playing->m_audioEventRTS.Peek() && playing->m_audioEventRTS.Peek() == lowestPriorityEvent)
				{
...
void OpenALAudioManager::adjustVolumeOfPlayingAudio(AsciiString eventName, Real newVolume)
{
	...
	for (it = m_playingSounds.begin(); it != m_playingSounds.end(); ++it) {
		...
		if (playing && playing->m_audioEventRTS && playing->m_audioEventRTS->getEventName() == eventName) {
			...
			alSourcef(playing->m_source, AL_GAIN, getEffectiveVolume(playing->m_audioEventRTS.Peek()));
		}
	}
	...
	for (it = m_playingStreams.begin(); it != m_playingStreams.end(); ++it) {
		...
		if (playing && playing->m_audioEventRTS && playing->m_stream && playing->m_audioEventRTS->getEventName() == eventName) {
			// Adjust it
			playing->m_audioEventRTS->setVolume(newVolume);
			alSourcef(playing->m_stream->getSource(), AL_GAIN, getEffectiveVolume(playing->m_audioEventRTS.Peek()));
		}
	}
}
...
Bool OpenALAudioManager::startNextLoop(PlayingAudio* looping)
{
	...
	if (looping->m_audioEventRTS->hasMoreLoops()) {
		...
		if (looping->m_audioEventRTS->getDelay() > MSEC_PER_LOGICFRAME_REAL) {
			// fake it out so that this sound appears done
...
static Bool ggcCanUseNativeLoop(const AudioEventRTS* event)
{
	...
	if (info->m_delayMin != 0 || info->m_delayMax != 0)
	{
		return false;                                // delayed loops must return through the request scheduler
	}
...
//-------------------------------------------------------------------------------------------------
// TheSuperHackers @bugfix bobtista 30/07/2026 Use gapless queued playback automatically for
// zero-delay multi-sample loops that ggcCanUseNativeLoop cannot cover - "loop all random" beds with
// several body samples and/or an attack/decay (the Particle Cannon beam, unit engines, trains).
// Delayed loops must return through startNextLoop so their configured pause is honored. The
// GGC_AUDIO_QUEUED_LOOP override retains the original opt-in behavior for diagnostic comparisons.
static Bool ggcUseQueuedLoop(const AudioEventRTS* event)
{
	static const Bool forceEnabled = (getenv("GGC_AUDIO_QUEUED_LOOP") != NULL);
	const AudioEventInfo* info = event ? event->getAudioEventInfo() : NULL;
	if (info == NULL)
		return false;
	if (info->m_soundType == AT_Music || info->m_soundType == AT_Streaming)
	...
	if (info->m_sounds.empty())
		return false;
	if (!forceEnabled && (info->m_delayMin != 0 || info->m_delayMax != 0))
	{
		return false;                                // the queued path intentionally has no silence buffers
	}
	return true;
}
...
{
	AudioEventRTS* ev = playing->m_audioEventRTS.Peek();
	if (ev == NULL)
		return false;
...
void OpenALAudioManager::friend_forcePlayAudioEventRTS(const AudioEventRTS* eventToPlay)
{
	...
	RefCountPtr<DynamicAudioEventRTS> eventPtr;
	eventPtr.Assign_No_Add_Ref(newInstance(DynamicAudioEventRTS)(*eventToPlay));
	AudioEventRTS* event = eventPtr.Peek();
