...
W3DMouse::~W3DMouse()
{
	...
	{
		WW3D::Get_Render_Backend()->Show_Hardware_Cursor(false);	//kill DX8 cursor
...
void W3DMouse::setCursor( MouseCursor cursor )
{
	...
	if (m_currentRedrawMode == RM_DX8)
	{
		...
		if (WW3D::Get_Render_Backend() != nullptr)
		{
			WW3D::Get_Render_Backend()->Show_Hardware_Cursor(false);	//disable DX8 cursor
		...
		if (doImageChange)
		{
			...
			WW3D::Get_Render_Backend()->Set_Hardware_Cursor_Image(
				m_currentHotSpot.x, m_currentHotSpot.y,
				m_currentD3DSurface[(Int)m_currentAnimFrame]);
			WW3D::Get_Render_Backend()->Show_Hardware_Cursor(true);	//Enable DX8 cursor
	...
	if (m_currentRedrawMode == RM_DX8 && m_currentD3DCursor != NONE)
	{
		...
		{	WW3D::Get_Render_Backend()->Show_Hardware_Cursor(true);	//Enable DX8 cursor
			...
			{	//if we're full-screen, need to manually move cursor image
				...
				WW3D::Get_Render_Backend()->Set_Hardware_Cursor_Position(ptCursor.x, ptCursor.y);
			}
