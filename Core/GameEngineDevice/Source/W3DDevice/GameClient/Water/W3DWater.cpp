...
static inline void W3DWater_BindTexture(unsigned stage, TextureClass * tex)
{
	...
	if (WW3D::Get_Render_Backend() != nullptr)
		WW3D::Get_Render_Backend()->Set_Texture(stage, tex);
}
...
void WaterRenderObjClass::ReAcquireResources()
{
	...
	{	//pixel/vertex shader based water assets.
		...
		m_pReflectionTexture = WW3D::Get_Render_Backend()->Create_Render_Target (SEA_REFLECTION_SIZE, SEA_REFLECTION_SIZE);
#endif
	}
...
void WaterRenderObjClass::drawSea(RenderInfoClass & rinfo)
{
	...
	WW3D::Get_Render_Backend()->Set_Texture(0,nullptr);	//we'll be setting our own textures, so reset W3D
	WW3D::Get_Render_Backend()->Set_Texture(1,nullptr);	//we'll be setting our own textures, so reset W3D
...
//	m_pDev->SetTextureStageState( 1, D3DTSS_MIPFILTER, D3DTEXF_NONE );
	...
	if (WW3D::Get_Render_Backend()) WW3D::Get_Render_Backend()->Set_Texture(0, nullptr); // raw D3D texture, clear bgfx cache
#ifdef MIPMAP_BUMP_TEXTURE
	m_pDev->SetTextureStageState( 0, D3DTSS_MIPFILTER, D3DTEXF_POINT );
	m_pDev->SetTextureStageState( 0, D3DTSS_MINFILTER, D3DTEXF_LINEAR );
	m_pDev->SetTextureStageState( 0, D3DTSS_MAGFILTER, D3DTEXF_LINEAR );
...
//			vb->v2=m_initialGridV2+(float)j*vScale2;
	...
	WW3D::Get_Render_Backend()->Set_Texture(0,setting->waterTexture);
	WW3D::Get_Render_Backend()->Set_Texture(1,setting->waterTexture);
...
#define HEIGHT_TO_USE (0.5f)
	...
	WW3D::Get_Render_Backend()->Override_Alpha_Blend_Enable(true);
	WW3D::Get_Render_Backend()->Override_Material_Opacity(0.5f);
