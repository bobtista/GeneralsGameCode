...
void WaterRenderObjClass::drawTrapezoidWaterBatch(const std::vector<WaterTrapezoidBatchEntry> &trapezoids)
{
	...
	while (batchStart < trapezoids.size())
	{
		...
		if (WW3D::Get_Render_Backend()->Has_Shader_Pipeline()
			&& WW3D::Get_Render_Backend()->Get_Back_Buffer_Format() == WW3D_FORMAT_A8R8G8B8
		...
		else if (!WW3D::Get_Render_Backend()->Has_Shader_Pipeline())
		{
			...
			WW3D::Get_Render_Backend()->Set_Color_Write_Enable(true, true, true, true);
		}
