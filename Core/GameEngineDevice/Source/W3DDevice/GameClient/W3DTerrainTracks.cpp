...
*/
	...
	if (m_edgesToFlush >= 2)
	{
		...
		// TheSuperHackers @perf bobtista 03/06/2026 Coalesce per-module track
		// draws into one Draw_Triangles per stage-zero texture. All tracks share
		// one vertex buffer and one world transform; only the texture differs, so
		// we build a per-flush index buffer of ABSOLUTE vertex indices grouped by
		// texture (with no bridge quad between modules) and emit a single draw per
		// texture instead of one per module. The per-quad index layout reproduces
		// the static strip pattern bit-for-bit, so rendered pixels are identical
		// within a texture. The bgfx->DX11 per-draw submission cost is the render
		// bottleneck (see memory project_bgfx_perf_levers), so fewer draws is the
		// only effective lever. The DX8 reference path keeps the per-module loop.
		Int totalVerts=0;
		Int totalTris=0;
