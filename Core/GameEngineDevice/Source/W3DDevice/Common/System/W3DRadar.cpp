...
void W3DRadar::renderObjectList( const RadarObject *listHead, TextureClass *texture )
{
	...
	if (WW3D::Get_Render_Backend() != nullptr && texture != nullptr)
...
void W3DRadar::setShroudLevel(Int shroudX, Int shroudY, CellShroudStatus setting)
{
	...
	if (m_shroudSurface == nullptr)
	{
		...
		if (WW3D::Get_Render_Backend() != nullptr && m_shroudTexture != nullptr)
			WW3D::Get_Render_Backend()->Invalidate_Cached_Texture(m_shroudTexture);
	}
	else
	{
		// This is cheap.
