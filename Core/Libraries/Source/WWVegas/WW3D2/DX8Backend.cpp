...
// In-engine back-buffer screenshot for dx8, mirroring the bgfx
// GGC_BGFX_SCREENSHOT_AFTER/PATH mechanism. PrintWindow + GDI fail to capture
// D3D8 surfaces (they don't compose via DWM), so we use the existing
// Capture_Back_Buffer_Image and write a 32-bit BMP. Env vars are read once.
// Persistent user env vars are required since process-scope vars do not
// reach the game; see memory `env-var-propagation-gotcha`.
//   GGC_DX8_SCREENSHOT_AFTER=<N>   — first capture at frame N
//   GGC_DX8_SCREENSHOT_INTERVAL=<K>— repeat every K frames (optional)
//   GGC_DX8_SCREENSHOT_PATH=<base> — writes <base>.<frameIndex>.bmp
