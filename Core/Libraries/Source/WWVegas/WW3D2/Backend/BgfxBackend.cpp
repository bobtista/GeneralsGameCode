                                     ...
                                     unsigned short vertex_count)
{
    ...
    // set by the caller via WW3D::Get_Render_Backend()->Set_Transform).
...
// the WW3D::Get_Render_Backend() abstraction so callers (water code in particular)
                      ...
                      unsigned short vertex_count)
{
    ...
    // WW3D::Get_Render_Backend(). Poll the DX8 state to keep g_renderToTexture in sync.
    ...
    // Set_DX8_Texture_Stage_State which bypasses WW3D::Get_Render_Backend(), so
    ...
    // bypassing WW3D::Get_Render_Backend(). Reading the device state here ensures
    ...
    // WW3D::Get_Render_Backend()->Set_Cull_Mode().
    {
        unsigned d3dCull = DX8Wrapper::Get_DX8_Render_State(D3DRS_CULLMODE);
        state &= ~(BGFX_STATE_CULL_CW | BGFX_STATE_CULL_CCW);
        if (d3dCull == 2) // D3DCULL_CW
