...
IndexBufferClass::WriteLockClass::~WriteLockClass()
{
	...
	if (WW3D::Get_Render_Backend() != NULL && indices != NULL &&
		...
		WW3D::Get_Render_Backend()->Capture_Index_Data(index_buffer, indices, total_bytes);
	}
	switch (index_buffer->Type()) {
	case BUFFER_TYPE_DX8:
		DX8_Assert();
...
IndexBufferClass::AppendLockClass::~AppendLockClass()
{
	...
	if (WW3D::Get_Render_Backend() != NULL && indices != NULL &&
		...
		WW3D::Get_Render_Backend()->Capture_Index_Sub_Range(index_buffer, indices, AppendStartIndex, size_bytes);
	}
	switch (index_buffer->Type()) {
	case BUFFER_TYPE_DX8:
		DX8_Assert();
