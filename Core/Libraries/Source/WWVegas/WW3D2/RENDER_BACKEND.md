...
| 3 | Port rendering subsystems to use the interface, in order of isolation: 2D UI, debug geometry, heat haze, static meshes, terrain, shadows, water, particles. **Scope revision**: this phase is now a pure decoupling pass (no backend runtime changes). Filling in real backend implementations is deferred to Phase 4 because of the DX8 swapchain ownership problem — see [PHASE3.md](PHASE3.md) for details. | **9 subsystems done** (5 in Phase 3, 3 + interface extension in Phase 3B, W3DShroud via SurfaceClass in Phase 3C on `bobtista/refactor/phase3c-shroud`, see [PHASE3.md](PHASE3.md) / [PHASE3B.md](PHASE3B.md) / [PHASE3C.md](PHASE3C.md)) |
