...
| 3 | Port rendering subsystems to use the interface, in order of isolation: 2D UI, debug geometry, heat haze, static meshes, terrain, shadows, water, particles. **Scope revision**: this phase is now a pure decoupling pass (no backend runtime changes). Filling in real backend implementations is deferred to Phase 4 because of the DX8 swapchain ownership problem — see [PHASE3.md](PHASE3.md) for details. | **First batch done** (5 subsystems migrated on branch `bobtista/refactor/phase3-migrate-callsites`, see [PHASE3.md](PHASE3.md)) |
