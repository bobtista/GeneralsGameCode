...
static void Apply_Render_State(RenderStateStruct& render_state)
{
	...
	// sorted batch state through WW3D::Get_Render_Backend() so the bgfx backend
	...
	WW3D::Get_Render_Backend()->Capture_Sorted_Batch_Transforms(
  ...
  //prevLight = render_state.lightsHash;

	{
		...
		WW3D::Get_Render_Backend()->Capture_Sorted_Batch_Light(rbLight, render_state.LightEnable[0]);
	}
...
void SortingRendererClass::Flush()
{
	...
	while (!sorted_list.empty()) {
		...
		if ((state->sorting_state.index_buffer_type==BUFFER_TYPE_SORTING || state->sorting_state.index_buffer_type==BUFFER_TYPE_DYNAMIC_SORTING) &&
			...
			WW3D::Get_Render_Backend()->Set_Shader(state->sorting_state.shader);
			WW3D::Get_Render_Backend()->Set_Material(state->sorting_state.material);
			for (int t = 0; t < DX8Wrapper::Get_Current_Caps()->Get_Max_Textures_Per_Pass(); ++t)
			{
				WW3D::Get_Render_Backend()->Set_Texture(t, state->sorting_state.Textures[t]);
			}
			...
			// Restore DX8 render state: the WW3D::Get_Render_Backend() calls above
