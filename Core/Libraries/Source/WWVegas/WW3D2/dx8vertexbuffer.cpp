...
VertexBufferClass::WriteLockClass::~WriteLockClass()
{
	...
	if (WW3D::Get_Render_Backend() != NULL && Vertices != NULL &&
		...
		WW3D::Get_Render_Backend()->Capture_Vertex_Data(VertexBuffer, Vertices, total_bytes);
	}
	switch (VertexBuffer->Type()) {
	case BUFFER_TYPE_DX8:
#ifdef VERTEX_BUFFER_LOG
...
VertexBufferClass::AppendLockClass::~AppendLockClass()
{
	...
	if (WW3D::Get_Render_Backend() != NULL && Vertices != NULL &&
		...
		WW3D::Get_Render_Backend()->Capture_Vertex_Sub_Range(VertexBuffer, Vertices, AppendStartIndex, size_bytes);
	}
	switch (VertexBuffer->Type()) {
	case BUFFER_TYPE_DX8:
		DX8_Assert();
