...
{
    // TheSuperHackers @perf bobtista 03/06/2026 GGC_BGFX_RENDERER A/Bs backends
    // at run time on any platform: dx11/d3d11, dx12/d3d12, vulkan, metal, gl.
    // On Windows this allows testing Direct3D12 (lower per-draw CPU overhead);
    // bgfx's DX11 and DX12 backends share SM5 DXBC shaders so no recompile is
    // needed. NOTE as of 03/06/2026: requesting dx12 falls back to DX11 because
    // bgfx's DX12 device creation fails on this setup (the reason needs a debug
    // bgfx build to extract - release bgfx compiles out BX_TRACE). On macOS,
    // Apple's Metal JIT (AGX/MTLCompiler) has intermittent crash bugs, so "gl"
    // is a stable fallback there.
...
static void SetupSunShadowView()
{
    ...
    // TheSuperHackers @bugfix bobtista 18/06/2026 Size the ortho to the visible ground footprint
    // plus a small margin - NOT a large fixed margin. A caster and its ground shadow share the same
    // light-space right/up position (the sun direction is the ortho's depth axis), so any caster
    // whose shadow lands on the visible ground already has its R/U inside the footprint and can only
    // be clipped in depth - and the depth range (2*D = 4000) is huge, so it never clips. A tall part
    // that scrolled off the top (Command Center dish, Strategy Center antenna) is far in 3D distance
    // mostly because of its height (Z), but its R/U is still within the footprint, so a footprint-fit
    // ortho with a small edge margin covers its shadow. An oversized ortho (the previous 800-unit
    // margin) made the map cover far more than the screen, collapsing on-screen shadow resolution so
    // thin shadows washed out and vanished. The small margin catches shadows right at the screen edge.
...
void BgfxBackend::Shutdown()
{
    ...
    if (g_device.initialized)
    {
        ...
        // TheSuperHackers @bugfix bobtista 02/06/2026 Unbind every view's framebuffer and
        // pump several empty frames before destroying any GPU resources. The textures and
        // buffers we destroy below are still referenced by the last gameplay frame's
        // command buffers and by view-framebuffer attachments; destroying them while those
        // references are live makes bgfx emit a "RefCount is 1 (expected 0)" warning per
        // resource even though the destroy itself succeeds. Pumping empty frames retires the
        // last frame's command buffers (bgfx keeps a few in flight) so the destroys are clean.
        ...
        // but if shutdown beats teardown they leak.
        // TheSuperHackers @bugfix bobtista 02/06/2026 Also destroy texture entries the same
        // way Destroy_Resource does. Two flavors exist: entries from Register_Texture_Resource
        // keep entry.fb/entry.texture invalid (the handle lives in g_caches.texture, drained
        // above) so the isValid guards make them a safe no-op; entries from Create_Texture own
        // a standalone entry.texture (or a render-target entry.fb) that no cache holds, and
        // were previously leaked at shutdown. Destroy directly here, not via the deferred
        // queue, which was already drained above.
...
void DeferDestroyStaticVertexResource(BgfxPhase5Entry & entry)
{
    if (!bgfx::isValid(entry.vb))
    {
        return;
...
void DeferDestroyStaticIndexResource(BgfxPhase5Entry & entry)
{
    if (!bgfx::isValid(entry.ib))
    {
        return;
...
void BgfxBackend::Capture_Legacy_Render_State_For_Sorted_Draw(RenderStateStruct & state)
{
    ...
    // TheSuperHackers @bugfix bobtista 25/05/2026 The police-car lightbar uses
    // small CopLight*.tga glow meshes attached to animated pivots that move the
    // active glow up to the roof and the inactive glows down inside the body.
    // Capturing the live per-mesh world is what lets bgfx place each glow at
    // the bone's animated position instead of model origin.
