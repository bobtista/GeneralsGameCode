...
bool CollisionMath::Intersection_Test(const SphereClass& sphere, const AABoxClass& box)
{
	...
	*/
	...
	if (WWMath::Fabsf(dc.X) < box.Extent.X + sphere.Radius)
		return false;
	if (WWMath::Fabsf(dc.Y) < box.Extent.Y + sphere.Radius)
		return false;
	if (WWMath::Fabsf(dc.Z) < box.Extent.Z + sphere.Radius)
		return false;
	return true;
}
...
bool CollisionMath::Intersection_Test(const SphereClass& sphere, const OBBoxClass& box)
{
	...
	*/
	...
	if (box.Extent.X < WWMath::Fabsf(box_rel_center.X))
		return false;
	if (box.Extent.Y < WWMath::Fabsf(box_rel_center.Y))
		return false;
	if (box.Extent.Z < WWMath::Fabsf(box_rel_center.Z))
		return false;
