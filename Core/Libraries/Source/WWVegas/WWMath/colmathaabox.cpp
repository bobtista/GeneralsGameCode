...
CollisionMath::OverlapType CollisionMath::Overlap_Test(const AABoxClass& box, const SphereClass& sphere)
{
	...
	//
	if (WWMath::Fabsf_Legacy(dist.X) > extent.X)
		return OUTSIDE;
	if (WWMath::Fabsf_Legacy(dist.Y) > extent.Y)
		return OUTSIDE;
	if (WWMath::Fabsf_Legacy(dist.Z) > extent.Z)
		return OUTSIDE;
...
CollisionMath::OverlapType CollisionMath::Overlap_Test(const AABoxClass& box, const LineSegClass& line)
{
	...
	else
	{
		...
		if (WWMath::Fabsf_Legacy(p0_proj) > box_proj)
			return OUTSIDE;
