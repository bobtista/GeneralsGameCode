...
{
	if (WWMath::Fabsf(point.X - box.Center.X) > box.Extent.X)
		return POS;
	if (WWMath::Fabsf(point.Y - box.Center.Y) > box.Extent.Y)
		return POS;
	if (WWMath::Fabsf(point.Z - box.Center.Z) > box.Extent.Z)
		return POS;
...
WWINLINE CollisionMath::OverlapType CollisionMath::Overlap_Test(const AABoxClass& box, const AABoxClass& box2)
{
	...
	if (box.Extent.X + box2.Extent.X < WWMath::Fabsf(dc.X))
		return POS;
	if (box.Extent.Y + box2.Extent.Y < WWMath::Fabsf(dc.Y))
		return POS;
	if (box.Extent.Z + box2.Extent.Z < WWMath::Fabsf(dc.Z))
		return POS;
