...
WWProfileHierarchyNodeClass		WWProfileManager::Root( "Root", NULL );
...
void WWProfileManager::Increment_Frame_Counter( void )
{
	if (ProfileCollecting) {
		...
		WWProfileHierarchyNodeClass* new_root=Root.Clone_Hierarchy(NULL);
...
static unsigned Read_Frame(char* memory,unsigned pos,unsigned maxpos,WWProfileHierarchyInfoClass*& root,HashTemplateClass<unsigned, StringClass>& id_hash)
{
	...
	float frametime;
	root=NULL;
	WWProfileHierarchyInfoClass* parent=NULL;
	WWProfileHierarchyInfoClass* latest=NULL;
	...
	if (file != NULL && file->Is_Available()) {
		...
		while (pos<size) {
			...
			if (tmp[0]=='I' && tmp[1]=='D' && tmp[2]==':') {
				...
				WWProfileHierarchyInfoClass* node=NULL;
