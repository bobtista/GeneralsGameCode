...
void DisconnectManager::update(ConnectionManager *conMgr) {
	...
	if (TheGameLogic->getFrame() == m_lastFrame) {
		...
		if ((curTime - m_lastFrameTime) > TheGlobalData->m_networkDisconnectTime) {
			...
			// TheSuperHackers @tweak bobtista 11/09/2026 The retail countdown and kick run as
			// before while a peer is missing; the hold is started by the missing peer's rejoin
			// request (ConnectionManager::processRejoinRequest), not by a timer here, so a player
			// who never comes back is kicked and the game continues without them.
