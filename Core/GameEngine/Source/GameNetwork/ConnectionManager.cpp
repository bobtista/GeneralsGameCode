...
void ConnectionManager::resendRecoveryReady() {
	if (m_recoveryReadyCount[m_localSlot] == 0) {
		return;
	}
	const UnsignedInt round = m_recoveryReadyCount[m_localSlot] - 1;
	...
	for (Int peer = 0; peer < MAX_SLOTS; ++peer) {
		...
		Bool reported = FALSE;
		for (UnsignedInt held = 0; held < m_recoveryReadyCount[peer]; ++held) {
