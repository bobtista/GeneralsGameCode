...
// TheSuperHackers @feature bobtista 29/06/2026 JSON converter for SCB script files and MAP files
...
static bool decompressIfNeeded(std::string& data)
{
	...
	// TheSuperHackers @bugfix bobtista 29/06/2026 Reject an absurd declared size from a corrupt
	// header before allocating, to avoid a multi-GB allocation / bad_alloc on malformed input.
	const Int maxUncompLen = 256 * 1024 * 1024;  // 256 MB ceiling; real script/map files are far smaller
	if (uncompLen > maxUncompLen)
	{
		fprintf(stderr, "Error: Refusing to decompress: declared size %d exceeds %d byte limit\n",
		        uncompLen, maxUncompLen);
		return false;
	}
...
// TheSuperHackers @feature bobtista 29/06/2026 Parse ScriptTeams chunk - stores team Dicts for JSON export
...
// TheSuperHackers @feature bobtista 29/06/2026 Check a single script for semantic issues
...
// TheSuperHackers @feature bobtista 29/06/2026 Check all scripts in a list for semantic issues
...
// TheSuperHackers @feature bobtista 29/06/2026 Perform semantic validation on all script lists
