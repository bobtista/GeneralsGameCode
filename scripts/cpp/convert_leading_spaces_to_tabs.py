...
def process_file(filepath, dry_run=False, verbose=False):
    ...
    for line_idx, line in enumerate(lines):
        ...
        # Require 2+ trailing spaces — a single space is a stray, not alignment.
        if leading_ws[0] == '\t':
        ...
        # Mixed whitespace with spaces before tabs: tabs indicate the real depth.
        ...
        # Ignore /* inside line comments and string literals — only scan the
        # code portion up to a //.
        code_part = stripped
        line_comment = code_part.find('//')
        if line_comment >= 0:
            code_part = code_part[:line_comment]
