                                                            ...
                                                            CommandEvaluateType type)
{
	...
	else if (TheInGameUI->areSelectedObjectsControllable() || (command && command->getCommandType() == GUI_COMMAND_SPECIAL_POWER_FROM_COMMAND_CENTER))
	{
		...
		if (command &&
		    (command->isContextCommand() || command->getCommandType() == GUI_COMMAND_SPECIAL_POWER || command->getCommandType() == GUI_COMMAND_SPECIAL_POWER_FROM_COMMAND_CENTER))
		{
...
GameMessageDisposition CommandTranslator::translateGameMessage(const GameMessage* msg)
{
	...
	switch (t)
	{
				...
				if (selectedObject->isLocallyControlled())
				{
					...
					for (temp; temp != selectedDrawable; temp = temp->getNextDrawable())
					{
						...
						if (temp == nullptr)
						{
							...
							if (tempObject && temp->getNextDrawable() == selectedDrawable && !temp->isSelected() && tempObject->isMobile() && tempObject->isLocallyControlled() && !tempObject->isContained() && !tempObject->isKindOf(KINDOF_DRONE))
							{
								newDrawable = temp;
								break;
							}
		...
		*/
		...
		{
			const CommandButton* command = TheInGameUI->getGUICommand();
			if (TheInGameUI->getSelectCount() > 0 || (command && command->getCommandType() == GUI_COMMAND_SPECIAL_POWER_FROM_COMMAND_CENTER))    // If something is selected
			{
		...
		case GameMessage::MSG_MOUSE_RIGHT_CLICK:
		{
			...
			if (TheGlobalData->m_useAlternateMouse && TheMouse->isClick(&m_mouseRightDragAnchor, &m_mouseRightDragLift, m_mouseRightDown, m_mouseRightUp))
			{
				...
				const CommandButton* command = TheInGameUI->getGUICommand();
				Bool controllable = TheInGameUI->areSelectedObjectsControllable() || (command && command->getCommandType() == GUI_COMMAND_SPECIAL_POWER_FROM_COMMAND_CENTER);
